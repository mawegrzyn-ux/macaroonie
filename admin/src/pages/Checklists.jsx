...
import {
  ...
  Pencil, RotateCcw, Loader2, ClipboardList,
} from 'lucide-react'
...
function periodLabel(frequency, periodStart) {
...
function RunChecklistModal({ template, date, api, qc, onClose }) {
...
function TodayTab({ venueId, date, api, qc }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {running && (
        <RunChecklistModal template={running} date={date} api={api} qc={qc} onClose={() => setRunning(null)} />
      )}
    </div>
  )
}
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      ...
      {!venueId ? (
        ...
        <TodayTab venueId={venueId} date={date} api={api} qc={qc} />
      ) : (
