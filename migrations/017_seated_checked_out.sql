-- ============================================================
-- 017_seated_checked_out.sql
--
-- Rename 'completed' → 'seated' (guest is at the table)
-- Add 'arrived'     (guest arrived, waiting to be seated)
-- Add 'checked_out' (guest has left, table is free)
--
-- Status flow:
--   confirmed → arrived → seated → checked_out
-- ============================================================

ALTER TYPE booking_status RENAME VALUE 'completed' TO 'seated';
ALTER TYPE booking_status ADD VALUE IF NOT EXISTS 'arrived';
ALTER TYPE booking_status ADD VALUE IF NOT EXISTS 'checked_out';

-- checked_out means table is physically free — exclude from capacity counts
-- (updates the exclusion list added in migration 016)
...
BEGIN
  ...
  IF FOUND THEN
    IF v_exception.is_closed THEN RETURN; END IF;
    ...
    IF FOUND THEN
      IF NOT v_exc_template.is_open THEN RETURN; END IF;
  ...
  FOR v_sitting IN (
    SELECT id, opens_at, closes_at, default_max_covers, sort_order, 'exception' AS source
      FROM exception_sittings
     WHERE template_id = v_exc_template_id AND v_use_exception = true
    UNION ALL
    SELECT id, opens_at, closes_at, default_max_covers, sort_order, 'override' AS source
      FROM override_sittings
     WHERE override_id = v_override.id AND v_use_override = true
    UNION ALL
    SELECT id, opens_at, closes_at, default_max_covers, sort_order, 'template' AS source
      FROM venue_sittings
     WHERE template_id = (
             SELECT id FROM venue_schedule_templates
              WHERE venue_id = p_venue_id AND day_of_week = EXTRACT(DOW FROM p_date)::smallint
           )
       AND NOT v_use_exception AND NOT v_use_override
    ...
    WHILE v_slot_time < v_sitting.closes_at LOOP
      ...
      IF (v_slot_end AT TIME ZONE v_venue.timezone)::time > v_sitting.closes_at THEN
        v_slot_time := v_slot_time + (v_interval || ' minutes')::interval; CONTINUE;
      END IF;
      ...
      SELECT sc.max_covers INTO v_cap_row
        FROM (
          SELECT max_covers FROM exception_sitting_slot_caps
           WHERE sitting_id = v_sitting.id AND slot_time = v_slot_time AND v_sitting.source = 'exception'
          UNION ALL
          SELECT max_covers FROM sitting_slot_caps
           WHERE sitting_id = v_sitting.id AND slot_time = v_slot_time AND v_sitting.source = 'template'
          UNION ALL
          SELECT max_covers FROM override_slot_caps
           WHERE sitting_id = v_sitting.id AND slot_time = v_slot_time AND v_sitting.source = 'override'
        ) sc LIMIT 1;

      IF FOUND THEN v_max_covers := v_cap_row.max_covers;
      ELSE          v_max_covers := v_sitting.default_max_covers;
      END IF;
      ...
      IF v_max_covers IS NOT NULL AND v_max_covers = 0 THEN
        IF v_venue.zero_cap_display = 'hidden' THEN
          v_slot_time := v_slot_time + (v_interval || ' minutes')::interval; CONTINUE;
        ELSE
          v_result.slot_time := v_slot_ts; v_result.available := false;
          v_result.available_covers := 0; v_result.reason := 'unavailable';
          RETURN NEXT v_result;
          v_slot_time := v_slot_time + (v_interval || ' minutes')::interval; CONTINUE;
        END IF;
      END IF;

      -- Active bookings only: cancelled, no_show, and checked_out all free the table
      ...
      IF v_max_covers IS NULL THEN
        v_result.available := true; v_result.available_covers := NULL; v_result.reason := 'available';
      ELSIF v_booked >= v_max_covers THEN
        v_result.available := false; v_result.available_covers := 0; v_result.reason := 'full';
      ELSIF (v_max_covers - v_booked) < p_covers THEN
        v_result.available := false; v_result.available_covers := v_max_covers - v_booked; v_result.reason := 'full';
      ELSE
        v_result.available := true; v_result.available_covers := v_max_covers - v_booked; v_result.reason := 'available';
      END IF;
