-- ============================================================
-- 015_fix_get_available_slots.sql
--
-- Fixes PostgreSQL error 55000:
--   "record v_exc_template is not assigned yet"
--
-- Root cause: in get_available_slots(), when no schedule_exception
-- covers the requested date the SELECT INTO v_exc_template never
-- executes, leaving the RECORD variable in an uninitialised state.
-- The UNION ALL sittings loop then tries to evaluate v_exc_template.id
-- which raises "record is not assigned yet" even though
-- v_use_exception = false would have excluded those rows.
--
-- Fix: replace the uninitialized record field access with a plain
-- uuid variable (v_exc_template_id) that safely defaults to NULL.
-- NULL in a WHERE clause produces no matching rows without an error.
-- ============================================================
...
DECLARE
  ...
  -- Scalar copy of v_exc_template.id — safe as NULL when no exception template applies.
  -- Accessing a field on an uninitialized PG RECORD raises 55000; a NULL uuid does not.
...
BEGIN
  ...
  -- Higher priority wins; narrower date range wins on ties.
  ...
  IF FOUND THEN
    ...
    IF FOUND THEN
      ...
      v_exc_template_id := v_exc_template.id;  -- store as safe scalar
    END IF;
  ...
  -- v_exc_template_id is NULL when no exception template applies → no rows from
  -- exception_sittings (WHERE template_id = NULL never matches).
  -- v_override is always assigned by SELECT INTO before this point when
  -- v_use_override = true, so v_override.id is safe there.
  FOR v_sitting IN (
    SELECT id, opens_at, closes_at, default_max_covers, sort_order,
           'exception' AS source
      FROM exception_sittings
     WHERE template_id    = v_exc_template_id   -- NULL-safe scalar
