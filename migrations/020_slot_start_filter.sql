-- ============================================================
-- 020_slot_start_filter.sql
--
-- Changes the slot availability rule so that a slot is shown
-- as long as the booking *starts* at or before closes_at
-- (last order time), even if the booking runs past it.
-- Previously, slots were only shown when the booking completed
-- before closes_at (slot_time + duration ≤ closes_at).
--
-- Also adds two new fields to slot_result so the API can
-- return sitting boundary times to the frontend:
--   sitting_closes_at   time  — the last order time for this sitting
--   sitting_doors_close time  — doors close time (may be NULL)
--
-- The frontend uses these to warn operators when a booking will
-- run past either boundary, without blocking the booking.
-- ============================================================

-- Drop dependent function before dropping the type
DROP FUNCTION IF EXISTS get_available_slots(uuid, date, int);
DROP TYPE IF EXISTS slot_result;
...
CREATE TYPE slot_result AS (
  ...
  reason              text,
  sitting_closes_at   time,       -- last order time for the sitting
  sitting_doors_close time        -- doors close time (nullable)
);
...
BEGIN
  ...
  FOR v_sitting IN (
    ...
    WHILE v_slot_time < v_sitting.closes_at LOOP
      ...
      -- The WHILE condition (v_slot_time < closes_at) already
      -- ensures the last slot starts before the last order time.
