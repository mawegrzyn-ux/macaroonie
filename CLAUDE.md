...
- `src/routes/leads.js` — Public (no auth) "register interest" capture for the marketing site at the apex domain (`/marketing/index.html`). `POST /api/leads` validates + saves to the global `leads` table (no RLS — prospects aren't tenants yet), then fire-and-forgets a notification email to `LEADS_NOTIFY_EMAIL` if set. Honeypot field (`company`, must be empty) for basic bot filtering; relies on the global Fastify rate limit for abuse protection, no route-specific limit.
...
### Marketing site
- `marketing/index.html` — Standalone static "register interest" landing page, served by Nginx directly from disk (no build step, no framework) at the **apex** domain (`macaroonie.com`). Self-contained HTML/CSS/JS. Posts the interest form to `POST /api/leads` (same-origin — apex still proxies `/api/` to Fastify). Four feature pillars: Bookings, Website, Online ordering (all shipped), HACCP & food safety (marked "Coming soon" — not built, see Outstanding items). `marketing/favicon.svg` is a copy of `admin/public/favicon.svg` — keep them in sync if the logo changes.
...
- ✅ **Admin portal moved to `office.macaroonie.com` + apex marketing page** *(migration 068, code-side — see Outstanding items for the manual deploy steps)*. The apex domain used to serve the admin/ops portal directly; it now serves a standalone static "register interest" landing page (`marketing/index.html`) instead, and the admin SPA moved to its own subdomain. Nothing in the admin app itself needed hardcoded-domain changes — `redirect_uri: window.location.origin` in `main.jsx` and the `widgetOrigin` "last-two-parts" apex-derivation heuristic in `dataBlocks.jsx` (already written to support `app.macaroonie.com`-style admin hosting) both generalise to `office.macaroonie.com` with no code change. Updated: `setup.sh` (apex block now roots at `marketing/`, new `office.${DOMAIN}` server block roots at `admin/dist` + proxies `/api/` and `/ws` — no `/reservations`/`/widget-api` needed there, see gotcha below; certbot now requests both `-d $DOMAIN -d office.$DOMAIN`), `RESERVED_SUBDOMAINS` in `siteRenderer.js` (added `office`), `Docs.jsx` Auth0 setup steps (Application Login URI + Allowed Callback/Logout/Web Origins now read `https://office.macaroonie.com`). New `POST /api/leads` endpoint + `leads` table (migration 068) back the landing page's form, with an optional email notification via `LEADS_NOTIFY_EMAIL`. Also fixed a pre-existing gap found while touching this block: the apex Nginx config had no `/manage` proxy location at all, so guest booking-management links (`manageBaseUrl` in `emailSvc.js` points at the apex) would have hit the admin SPA's `index.html` instead of the Fastify SSR page — the admin app has no client-side `/manage` route.
...
**9. `office.macaroonie.com` domain split — deployment pieces** *(feature shipped code-side, not yet deployed)*
Repo-side changes are done (`setup.sh`, `RESERVED_SUBDOMAINS`, `Docs.jsx`, `marketing/index.html`, `leads.js` + migration 068) but nobody with server/DNS/Auth0 access has run the manual steps yet:
- **DNS**: add an `office` A record (or CNAME) on Lightsail DNS pointing at the same server IP as the apex.
- **SSL**: if the wildcard cert from the "Lightsail wildcard cert setup runbook" gotcha is already live, it already covers `office.macaroonie.com` — just point the new Nginx server block at those existing cert files. Otherwise run `certbot --nginx -d macaroonie.com -d office.macaroonie.com` (see updated `setup.sh` for the exact call).
- **Nginx on the live server**: the live config has almost certainly drifted from `setup.sh` (see the many manual-tweak gotchas below), so don't blindly re-run the whole script. SSH in and hand-apply just the new pieces: apex `location /` + asset regex now root at `${APP_DIR}/marketing` instead of `${APP_DIR}/admin/dist`, the new `/manage` proxy location, and the new `server { server_name office.macaroonie.com; ... }` block — then `nginx -t && systemctl reload nginx`.
- **First deploy of `marketing/`**: nothing extra needed — it's static files already tracked in git, so the next `git reset --hard origin/main` in `deploy.yml` brings them onto the server automatically. It's the Nginx *root* pointing at that path that's the manual step above.
- **Auth0 dashboard** (Applications → Macaroonie Admin → Settings): set **Application Login URI** to `https://office.macaroonie.com`, and add it to **Allowed Callback URLs**, **Allowed Logout URLs**, and **Allowed Web Origins**. Keep the old `https://macaroonie.com` entries until the cutover is confirmed working, then remove them.
- **`LEADS_NOTIFY_EMAIL`**: optional but recommended — set it in the API's `.env` so "register interest" submissions actually notify someone (leads are saved to the DB either way).
...
- **The apex Nginx block had no `/manage` proxy location** — guest booking-management links (`manageBaseUrl` in `emailSvc.js` = `PUBLIC_ROOT_DOMAIN`, i.e. the apex) point at `/manage/{token}`, which is SSR'd server-side by `manageBooking.js`. The admin SPA has no client-side route for that path, so without an explicit Nginx proxy location, the request would hit the SPA's `try_files ... /index.html` fallback and render the wrong app entirely. Found and fixed while moving the admin portal to `office.macaroonie.com` (`setup.sh`) — any new apex-facing SSR route needs its own Nginx location the same way `/webhooks/` and `/manage` have one; it will NOT fall through from the SPA's catch-all.
- **`widgetOrigin`'s "last-two-parts" heuristic in `dataBlocks.jsx` already anticipated admin-on-a-subdomain hosting** — it strips the admin's own hostname down to its last two labels (`parts.slice(-2).join('.')`) to derive the apex before building the tenant-subdomain iframe URL, specifically because the code comment already called out `app.macaroonie.com`-style admin hosting as a possibility. This is why moving the admin portal to `office.macaroonie.com` needed zero changes to the widget-preview iframe logic — it was written to survive exactly this move. Don't "simplify" this to `window.location.hostname` directly if the admin ever moves to a domain with more than two labels under the public suffix (e.g. `admin.ops.macaroonie.com`) — the heuristic would need a third label stripped.
