...
- ✅ **Timeline grey column overlay** — `GET /slots?covers=1` used to compute unavailable time strips; grey = outside sittings or cap=0 (not fully-booked); strips clipped in secondary combo rows to avoid bleed-through the spanning card
...
- **Timeline grey strips are per-column, not per-row** — grey = outside any sitting OR slot cap explicitly set to 0 (reason='unavailable'). Fully-booked slots (reason='full') are NOT greyed — the column stays white so operators can see there are bookings but can still use manual allocation to add more. Secondary combo rows clip grey strips to the booking's own time window only; grey IS shown outside the booking's time.
