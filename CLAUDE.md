...
- **Website hours default to the venue schedule** — `website_config.opening_hours_source` defaults to `venue` (sittings on `venue_schedule_templates`). `manual` is only for operators who want advertised hours that differ from booking hours. Empty `website_opening_hours` is not a source of truth. GET `/website/opening-hours` uses the same `loadOpeningHours` helper as SSR so the canvas preview matches the live site.
