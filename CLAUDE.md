...
- `src/views/site/blocks/` — per-block-type SSR partials. `renderer.eta` switches on `it.block.type` and includes the matching partial (`hero`, `text`, `image`, `two_column`, `cta_strip`, `gallery`, `opening_hours`, `find_us`, `contact`, `booking_widget`, `menu_pdfs`, `allergens`, `faq`, `divider`, `columns`). All sectional partials read `data.container` (boxed | wide | full) and emit the matching max-width. `columns.eta` is the only container partial — it iterates `d.columns[i].blocks[]` and re-includes `./renderer` per child for nested rendering, then emits a per-instance media-query class for mobile stacking.
...
- `src/pages/Website.jsx` — Website builder admin page (`/website` route). Per-venue mode left-rail (17 sections, top-to-bottom): **Page builder**, Setup & domain, Template, Theme overrides, Branding, Gallery, Menus (PDF), Allergens, Opening hours, Find us, Contact, Online ordering, Delivery, Booking widget, Custom pages, SEO, Analytics. Hero + About are **gone** — their content lives inside blocks added via the canvas. Brand-defaults mode left-rail (4 sections): Brand identity, Brand theme, Brand analytics, Emergency banner. Onboarding card for tenants with no `website_config` row yet. Shared primitives: `SectionCard`, `FormRow`, `Toggle`, `SaveBar`, `FileUpload`, `ImageField`, `useConfigFields()` helper. Theme section saves the full `theme` JSONB in one PATCH; each content section saves its own subset. Main-panel layout drops `max-w-3xl mx-auto` only when `active === 'page'` so the canvas can stretch — every other section stays constrained. GallerySection has a "From library" button that opens `MediaLibraryModal` in picker mode (in addition to fresh upload).
...
- `src/components/website-builder/PageBuilder.jsx` — top-level page-builder component for the website CMS. Owns the single page-level `DndContext` that coordinates every drag (top-level reorder, within-column reorder, cross-column moves, top↔column moves). Selection state (`selectedId`, `inspectorOpen`); fires `findBlock` / `reorderWithinParent` / `moveAcrossParents` from `blockTree.js`. Bails silently if a Columns block is dragged into a column (no nested columns). Templates picker modal (`PAGE_TEMPLATES` from `blockRegistry.js`) replaces the current tree on apply.
- `src/components/website-builder/blockRegistry.js` — single source of truth for block types. 15 entries (added `columns` container block). Each entry has `key`, `label`, `description`, `icon`, `category` (`hero` / `content` / `data` / `layout`), `defaultData`, `editor`, optional `pullsFromConfig`, optional `isContainer`. `CONTAINER_OPTIONS` (boxed/wide/full) + `NO_CONTAINER_BLOCKS` set (`{'divider'}`) + `DEFAULT_CONTAINER` exported. `newBlock(key)` mints a fresh `{ id, type, data }` with `crypto.randomUUID()` for both the block and any nested column ids. `PAGE_TEMPLATES` ships 4 starting templates (Restaurant Classic, Modern Bistro, Minimal Café, From scratch).
- `src/components/website-builder/blockTree.js` — pure helpers for the (now-nestable) block tree. Exports: `flattenBlocks`, `findBlock` (returns `{ block, parent }` where parent is `{ kind: 'top' }` or `{ kind: 'column', blockId, columnId }`), `patchBlock` / `patchBlockData` / `replaceBlock`, `removeBlock`, `duplicateBlock` (uses `cloneWithFreshIds`), `cloneWithFreshIds` (deep-clones + reassigns UUIDs to nested blocks AND columns), `insertAt`, `moveWithinParent`, `reorderWithinParent`, `moveAcrossParents`, `listForParent`, `parentKey` / `parseParentKey` (stable container ids — `'top'` or `'col:{blockId}:{columnId}'`). Treats top-level + nested blocks uniformly so the rest of the page builder doesn't branch on tree position.
- `src/components/website-builder/canvas/` — visual canvas for the page builder. Each block on the canvas is rendered as `BlockShell` (chrome) → `Canvas component` (visual mirror of the matching server `.eta`) → optional inline editors (`InlineText` / `InlineRichText`).
  - `BlockShell.jsx` — drag handle (left edge, fades in on hover), hover/selected outline (`2px solid var(--c-primary)`), floating toolbar above the selected block: move up/down, Settings, Duplicate, Delete (label badge on the left names the block type).
  - `BlockNode.jsx` — recursive renderer. Container blocks (currently just `columns`) receive a `renderChild` callback so they can recurse into `BlockNode` for their nested children — keeps shell-rendering logic in one place.
  - `BlockInserter.jsx` — Notion/Editor.js style "+" button between blocks (`mode='between'`) plus a dashed empty-state CTA (`mode='empty'`). Hover reveals a thin primary line; click opens a popover with category-grouped picker + search input.
  - `BlockInspector.jsx` — right-side settings panel (`w-[340px]`, sticky). First section is always the Container width toggle (Boxed / Wide / Full bleed) unless the block is in `NO_CONTAINER_BLOCKS`. Second section is the per-type editor reused from `editors/`. Closes on `×` or `Escape`.
  - `ThemeFrame.jsx` — wraps the canvas with the same CSS variables emitted by `views/site/shared/head.eta` (--c-*, --f-*, --r-*, --btn-*, etc), scoped to a per-instance class so the admin chrome's font/colours don't leak in. Auto-loads Google Fonts based on `theme.typography.heading_font` / `body_font`.
  - `themeResolver.js` — JS mirror of `head.eta`'s theme resolution. **When a public-side knob is added, update `head.eta` + this file + the admin `DEFAULT_THEME` in `Website.jsx` together.**
  - `InlineText.jsx` — `contentEditable` plain-text inline editor for headings, button labels, captions. Single-line by default (`Enter` blocked, pasted newlines stripped); pass `multiline` for subheadings/paragraphs. Sync-from-prop `useEffect` only writes into the DOM when `el.textContent !== value` AND the element is not focused — otherwise the caret would jump on every keystroke.
  - `InlineRichText.jsx` — TipTap-powered inline rich text. Floating format bar above the editor when focused: bold/italic/underline, H2/H3 (when `allowHeadings`), bullet/ordered list, blockquote, alignment, link (with `isSafeUrl()` URL validation — `http: | https: | mailto: | tel:`).
  - `blockCanvas.jsx` — per-type canvas components: `HeroCanvas`, `TextCanvas`, `ImageCanvas`, `TwoColumnCanvas`, `CtaStripCanvas`, `FaqCanvas`, `DividerCanvas`, `ColumnsCanvas`, `DataPlaceholderCanvas`. `innerContainerStyle(width)` mirrors the boxed/wide/full switch in every server `.eta`. `ImagePlaceholder` (empty state) and `ImagePicker` (overlay on a populated block) both upload via `POST /website/upload` and integrate `MediaLibraryModal`.
  - `canvasRegistry.js` — `block.type → canvas component` map. All "live data" blocks (gallery, opening_hours, find_us, contact, booking_widget, menu_pdfs, allergens) share `DataPlaceholderCanvas` (a striped placeholder labelled with the block's purpose — content comes from the existing per-section editors, not the block itself).
- `src/components/website-builder/editors/ColumnsEditor.jsx` — settings for the Columns block. 2/3/4-lane picker (shrinking merges dropped columns' children into the last surviving column so nothing is lost), gap (tight/normal/wide), vertical alignment (top/center/bottom/stretch), stack-on (mobile/tablet/never), background (default/surface/accent).
...
- ✅ **Visual page builder (canvas + inline editing)** — full rewrite of the website CMS page builder into a Gutenberg / Editor.js style canvas. Operators click any block to select it (highlighted with a primary outline + floating toolbar above for move up/down, Settings, Duplicate, Delete). Headings, button labels, captions, FAQ Q&A are edited directly on the canvas via `InlineText`; rich body content via `InlineRichText` (TipTap with floating format bar). Hover any block boundary for a `+` inserter that opens a category-grouped picker (Hero / Content / Layout / Live data) with search. Right-side `BlockInspector` reuses existing per-type `editors/` for layout/style options only — content fields are inline-only. New 15th block type `columns` (2–4 lanes); cross-column and column↔top-level drags work under a single page-level `DndContext` via `blockTree.moveAcrossParents()` + per-empty-column `useDroppable` zones. Every sectional block gained a `data.container` field (boxed | wide | full) that's read by both the admin canvas (`innerContainerStyle`) and the matching server `views/site/blocks/<type>.eta`. `ThemeFrame` scopes the public-site CSS variables to the canvas so what the operator sees matches what gets rendered; new mirror module `canvas/themeResolver.js` keeps that in sync with `views/site/shared/head.eta`. The new architecture lives under `admin/src/components/website-builder/canvas/` with a separate pure-helpers module `blockTree.js`. GallerySection gained a "From library" button next to upload that opens `MediaLibraryModal` in picker mode. Page builder is the first venue section in the left rail; Hero + About entries removed (their content lives inside blocks). Page-builder section runs full-width in the admin shell; other venue sections still wrap at `max-w-3xl`.
...
- **`canvas/ThemeFrame.jsx` is a JS mirror of `views/site/shared/head.eta`** — when a theme knob is added on the public side, update **three places in lockstep**: (1) `head.eta` CSS-var emit, (2) `canvas/themeResolver.js` (drives `ThemeFrame`), (3) the admin `DEFAULT_THEME` constant in `Website.jsx`. Forgetting the resolver leaves the canvas previewing stale fonts/colours; forgetting the default leaves the saved JSONB missing keys until a save.
- **`InlineText` is contentEditable, not controlled** — the sync-from-prop `useEffect` deliberately bails when the element is `document.activeElement` AND only writes into the DOM when `el.textContent !== value`. Reassigning on every keystroke caret-jumps to the start. Don't refactor this to a controlled `<input>` or to set `textContent` unconditionally — the focus-aware bypass is load-bearing.
- **Single page-level `DndContext` for the page builder** — `PageBuilder.jsx` owns it. `ColumnsCanvas` does NOT mount its own. A nested DndContext breaks cross-container drags (sortable items in the inner context don't see the outer context's drop targets and vice versa). This was the previous failure mode before the rewrite — don't reintroduce.
- **Columns can't nest inside Columns** — `handleAnyDragEnd` silently bails when `sourceInfo.block.type === 'columns' && destParent.kind === 'column'`. The `BlockInserter` inside a column also doesn't list `columns` in the picker (filter runs through `BLOCKS`, but the bail is the real guard). If a future requirement needs nested layouts, deepen the guard rather than removing it — letting a column drag into itself crashes the tree walker.
- **Empty columns need a `useDroppable` zone, not just per-block `useSortable`** — see `ColumnDropZone` in `blockCanvas.jsx`. Without it, drops onto an empty column do nothing because @dnd-kit only sees per-block drop targets. The wrapper provides an `id` (matches `parentKey()`) plus visual feedback (`isOver` outline) when a block is being dragged into the empty column.
- **`data.container` is per-block, not per-page** — boxed | wide | full. Lives on every sectional block's `defaultData` and is read by both `innerContainerStyle()` (admin canvas in `blockCanvas.jsx`) and the matching `views/site/blocks/<type>.eta`. When adding a new block type, default to `'boxed'` and add the same `cw / containerStyle` switch to the .eta partial. `divider` is the only block in `NO_CONTAINER_BLOCKS` so it doesn't show the toggle in the inspector.
- **`columns` IS in the container-width picker** — its `data.container` controls the outer row's max-width before the lanes split, so the operator can have a wide-bleed columns row inside an otherwise-boxed page. Inner column gaps and stacking are handled separately in `ColumnsEditor`.
- **`cloneWithFreshIds()` from `blockTree.js`, never `structuredClone` alone** — when duplicating a Columns block, `structuredClone` would copy nested block ids verbatim, then dnd-kit would silently misbehave (multiple nodes with the same sortable id). `cloneWithFreshIds` deep-clones AND assigns fresh `crypto.randomUUID()` to every nested block + column.
- **`BlockInspector` reuses per-type `editors/` components — content fields render in BOTH the inspector AND inline on the canvas** (heading, body_html, button labels, FAQ Q&A). They share the same `onChange` so editing in either place is reflected immediately. When adding a new block type, the inspector editor should focus on layout/style; content fields stay editable inline. Don't duplicate content state between the two — there's only one `data` object.
- **`Website.jsx` drops `max-w-3xl mx-auto` only when `active === 'page'`** — every other venue section still uses the constrained width. The form-heavy sections (theme, branding, SEO, etc.) rely on the narrow column for readability. Don't refactor the layout to drop the constraint globally.
- **Page builder section is the FIRST venue section, Hero + About are GONE from the sidebar** — their content lives inside blocks added via the canvas. The legacy flat-layout fallback was removed in the prior round. Don't add Hero/About back as separate sections — the design is "one block tree, edit content where it lives."
- **Adding a new block type is a 4-place change (was 3)** — (1) entry in `blockRegistry.js` with editor + defaultData (don't forget `container: 'boxed'` if sectional), (2) per-type partial under `views/site/blocks/<type>.eta` reading `d.container`, (3) line in `views/site/blocks/renderer.eta` for the dispatch, (4) line in `admin/src/components/website-builder/canvas/canvasRegistry.js` mapping the type → canvas component (and a matching component in `blockCanvas.jsx`). Skipping step 4 means the admin canvas falls through to "Unknown block type" while the public site renders fine.
