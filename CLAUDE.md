...
- **The Edit tool can introduce curly/typographic quote characters (U+2018 `'`, U+2019 `'`) as string delimiters** — these are NOT valid JavaScript string delimiters. esbuild throws `Unexpected "'"` pointing at the character position. Symptom looks identical to an encoding or quote-balance bug. Diagnosis: `python3 -c "open('file').read()" | hexdump -C | grep e2 80` — U+2018 is `e2 80 98`, U+2019 is `e2 80 99`. Fix pattern: always use double-quoted strings for new string literals in JSX/JS files, or apply Python byte-level replacement `content.replace(b'\xe2\x80\x98\xe2\x80\x99', b'""')`. This bit us in `ReservationsWidget.jsx` when the Edit tool wrote `confirmation_heading: '',` with curly quotes as delimiters (May 2026).
